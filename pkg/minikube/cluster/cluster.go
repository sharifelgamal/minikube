...
func createHost(api libmachine.API, config cfg.MachineConfig) (*host.Host, error) {
	...
	if !driver.BareMetal(config.VMDriver) {
		out.T(out.StartingVM, "Creating {{.driver_name}} VM '{{.machine_name}}' (CPUs={{.number_of_cpus}}, Memory={{.memory_size}}MB, Disk={{.disk_size}}MB) ...", out.V{"driver_name": config.VMDriver, "number_of_cpus": config.CPUs, "memory_size": config.Memory, "disk_size": config.DiskSize, "machine_name": config.Name})
	} else {
